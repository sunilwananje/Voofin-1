...
class CompanyRepo Implements CompanyInterface
{
        ...
        if ($validator->fails()) {
            ...
            try{
                ...
                if(isset($data->comp_type) && $data->comp_type == roleId('Buyer')){
                    $comp->configurations = '{"buyer":{"price_band":{"":""},"tax_configuration":{"VAT":"5","CST":"5"},"other_configuration":{"payment_terms":"","erp_integration":null},"maker_checker":{"po_creation":null,"invoice_approval":null,"pi_upload":null}}}';
                }
                ...
                elseif(isset($data->comp_type) && $data->comp_type == roleId('Both')){
                    $comp->configurations = '{"buyer":{"price_band":{"":""},"tax_configuration":{"VAT":"12","CST":"5"},"other_configuration":{"payment_terms":"","erp_integration":null},"maker_checker":{"po_creation":null,"invoice_approval":null,"pi_upload":null}},"seller":{"tax_configuration":{"VAT":"2","CST":"2"},"maker_checker":{"invoice_creation":"0","manual_discoutning":"1"},"other_configuration":{"auto_discounting":"1","auto_accept_po":"1"}}}';
                }
                $comp->save();
    ...
    public function sendEmailReminder($user) {
        Mail::send('emails.email', ['user' => $user], function ($m) use ($user) {
            $m->from('contact@geo.com', 'Your Application');
