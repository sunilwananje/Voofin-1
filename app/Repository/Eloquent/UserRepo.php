...
class UserRepo Implements UserInterface
{
    ...
    public function getData() {
        $query = User::orderBy('users.name', 'Asc')
                ->select('users.name as user_name', 'users.*');

        
        if ($this->email || $this->userId || $this->uuid) {            
            if ($this->email){
            ...
            if ($this->userId) $query->where('users.id', $this->userId);
            if ($this->uuid) $query->where('users.uuid', $this->uuid);
            ...
            if (isset($this->userType) && !empty($this->userType)) {
                ...
                if(is_array($this->userType) ){//if user is buyer or seller then userType must be array i.e.(buyer, both) this block must run
                    $query->join('roles', function ($join) {
                ...
                else{//if user is not buyer or seller then userType must be string i.e. bank or admin this block must run
                    $query->join('roles', function ($join) {
                        $join->on('roles.id', '=', 'users.role_id')
    ...
    public function save($data) {
        ...
        if ($data['uuid']) $rules = array('name' => 'required');
        else $rules = array('name' => 'required', 'email' => 'required|email|unique:users,email');
