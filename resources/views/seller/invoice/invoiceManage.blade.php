...
<form class="form-horizontal no-margin ng-valid-parse ng-invalid ng-invalid-required ng-valid-min ng-valid-pattern ng-pristine" name="invoiceForm" id="invoiceForm" role="form" action="{{ route('seller.invoice.save') }}" method="post" enctype="multipart/form-data" confirm-on-exit="" novalidate="">
      {{ csrf_field() }}
				<div class="row">
					...
					<div class="widget">
					    ...
					    <div class="widget-body dsk_mr80">
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required" id="invoice_number" name="invoice_number" id="invoice_number" placeholder="Invoice No" value="{{$invData->invoice[0]->invoice_number or old('invoice_number')}}" autocomplete="off">
							...
							<div class="form-group autocomplete_input">
								...
								<div class="col-sm-5" id="the-basics">
									<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required typeahead typeahead tt-query" id="buyer_name" name="buyer_name" placeholder="Buyer" autocomplete="on" spellcheck="false" value="{{$invData->invoice[0]->buyer_name or old('buyer_name')}}" autocomplete="off">									  
								    <input type="hidden" name="buyer_uuid" id="buyer_id" value="{{$invData->invoice[0]->buyer_uuid or old('buyer_uuid')}}">
								</div>									
							</div>
							...
							<div class="form-group autocomplete_input">
								...
								<div class="col-sm-5" id="the-basics">
									<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required typeahead typeahead tt-query" id="po_no" name="po_no" placeholder="Associated PO" autocomplete="on" spellcheck="false" value="{{$invData->invoice[0]->purchase_order_number or old('po_no')}}" autocomplete="off">									  
								    <input type="hidden" name="po_id" id="po_id" value="{{$invData->invoice[0]->po_uuid or old('po_id')}}">
								</div>									
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-5">
									<input type='text' name="in_due_date" id="in_due_date" class="form-control" placeholder="mm/dd/yyyy"  value="<?php if(isset($invData->invoice[0]->due_date)){ echo date('d/m/Y',strtotime($invData->invoice[0]->due_date));}else{ echo old('in_due_date');}?>" readonly autocomplete="off"/>
								</div>	
							</div>
							...
							<div class="form-group" id="currencyText" style="display:none">
								...
								<div class="col-sm-5">
									<input type='text' name="currency" id="currency" class="form-control" readonly value="{{$invData->invoice[0]->currency or old('currency')}}" autocomplete="off"/>
								</div>	
							</div>
							...
							<div class="form-group" id="currencyDrop">
								...
								<div class="col-sm-5">
                                   @if(isset($invData->invoice[0]->currency) && !empty($invData->invoice[0]->currency))
									 <input type='text' name="currency" id="currency" class="form-control" readonly value="{{$invData->invoice[0]->currency or ''}}"/>
									@else
									  <select name="currency" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required" id="curDrop">
							...
							<div class="form-group">
								...
								<div class="col-sm-5">									  	
									<textarea class="form-control" rows="5" name="remarks">{{$invData->invoice[0]->remarks or old('remarks')}}</textarea>									  
								</div>									
							</div>	
							<input type='hidden' class="form-control" name="invoice_uuid" value="{{$invData->invoice[0]->uuid or ''}}" />							
						</div>
					</div>
				</div>
				</div>
					...
					<div class="widget">
						...
						<div class="widget-body dsk_mr80">
							<div class="form-horizontal no-margin ng-valid-parse ng-invalid ng-invalid-required ng-valid-min ng-valid-pattern ng-pristine" name="addPoi" role="form" confirm-on-exit="" novalidate="">
								...
								@if(isset($invData->items) && count($invData->items)>0) <!-- for edit-->
								{{--*/ $subTotal = $invData->invoice[0]->amount-$invData->invoice[0]->discount /*--}} 
								  @forelse($invData->items as $key=>$item)
									<div class="form-group po_items">
										...
										<div class="col-sm-2">
											<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required price" id="priceper_0" name="price_per[]" placeholder="Price Per" value="{{$item->unit_price}}">
										</div>
								  ...
								  @elseif(old('item')!==null) <!-- for validation faild old value-->
								   ...
								   @else
								      <div class="form-group po_items"><!-- for default-->
											...
											<div class="col-sm-2">
												<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required price" id="priceper_0" name="price_per[]" placeholder="Price Per">
											</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required" id="subTotal" name="amount" placeholder="Sub Total" value="{{$invData->invoice[0]->amount or old('amount')}}" readonly>
									</div>	
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-2">
								        <select class="form-control" name="discount_type" id="discountType">
									      <option value="0" @if(isset($invData)) @if($invData->invoice[0]->discount_type==0) selected @endif @endif>Percentage</option>
									      <option value="1" @if(isset($invData)) @if($invData->invoice[0]->discount_type==1) selected @endif @endif>Absolute</option>
								        </select>
									</div>
									<div class="col-sm-2">
								        <input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required" id="discount" name="discount" placeholder="Discount" value="{{$invData->invoice[0]->discount or old('discount')}}">
									</div>		
								</div>
								...
								@if(isset($invData->invoice[0]->tax_details) && !empty($invData->invoice[0]->tax_details))
                                 @foreach(json_decode($invData->invoice[0]->tax_details) as $key=>$taxDet)
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										<input type="text" class="form-control ng-pristine ng-untouched ng-invalid ng-invalid-required" id="finalAmount" name="final_amount" placeholder="Total" value="{{$invData->invoice[0]->final_amount or old('final_amount')}}" readonly>
									</div>	
								</div>
							    ...
							    <div class="col-sm-4">
								  ...
								  <div class="col-sm-5">
								   <button type="submit" class="btn btn-primary pull-right">Save &amp; Send</button>
							      </div>
							    </div>
