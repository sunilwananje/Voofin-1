...
/*block related to PO ended here*/
 ...
 $(".invoice-modal").click(function(){
      var id=$(this).attr('id');
      $.ajax({
            url:'/buyer/invoice/invoiceDetails/'+id,
            dataType:'html',
            success:function(data){
              ...
              var taxTotal = 0, taxPer = 0 , subTotalDis=0, finalAmount=0;
               ...
               $('.taxes').each(function(){
                ...
                var taxPer=parseFloat($(this).val());
                
                taxPer = isNaN(taxPer) ? '0' : taxPer;
                
                taxTotal=taxTotal+((subTotal*taxPer)/100);
...
$("#purOrderNumber").on('blur',function(){
  ...
  $.ajax({
    ...
    success:function(msg){
      var value = eval(msg);
      if(value.success){
        $('#poOrderNumberMsg').text(value.success);
      }
      else if(value.error){
        $('#poOrderNumberMsg').html('');
      }
    }
  });
